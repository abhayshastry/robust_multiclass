...
def stochastic_kernel_matrix(exact_kernel_matrix, shots = 0, circuit_type = 'gates',seed = 0):# Use this instead of kernel_estimate
    np.random.seed(seed)
    if shots == 0:
        return exact_kernel_matrix
    exact_kernel_matrix = np.clip(exact_kernel_matrix, 0, 1)
...
def skm(exact_kernel_matrix, shots = 0, circuit_type = 'gates', N_trials = 100,seed=0):# Use this instead of kernel_estimate
...
def shifted_ksvm(K,y, shift = 0, C =  np.inf, svc_intercept = False, use_svc = True):#RSVM_QP is how it is named in the text
    n = len(y)
    ...
    if use_svc == False:
        delta=1
        print("Using dual_robust_quadratic program in shifted_ksvm. kappa(delta) term set to ",kappa(delta))
        alpha,b,SV = dual_robust_quadratic(K_shifted,y,delta=delta,C=C)
        return alpha,b,SV
    clf = SVC(kernel='precomputed',C = C)
    clf.fit(K_shifted, y)
    alpha_y =  clf.dual_coef_
    print("Using SVC in shifted_ksvm:", use_svc,". Using svc_intercept:",svc_intercept)
...
def primal_robust_socp(K_star, y_train, delta_1 = 1e-2, delta_2 = 1e-2, C = 1, shots = 20, circuit_type = 'gates',sv_threshold=5e-2,numerical_shift=1e-7,fix_psd=False):
    n = K_star.shape[1]
    N = 2*n + 2
    g = 0.5
#    if abs(eigmin(K_star))<numerical_shift:
#        K_star = K_star + numerical_shift*np.eye(n)
#    print("Is PSD?:",isPSD(K_star))
#    if fix_psd:
#        K_star = threshold_reg(K_star)
#    print("After fix:", isPSD(K_star)) 
    if circuit_type == 'swap':
        g = 1
    x = cp.Variable(N)
    #The SOCP Constraint:
    ...
    #h_rob.solve(solver='MOSEK')
    h_rob.solve(solver = 'SCS')
    primal_optimal_value = h_rob.value
    b = x[2*n].value
    beta = x[n:2*n].value #VECTOR. In previous formalism, symbol beta was used for nXn matrix
    scaled_beta = np.abs(beta)/np.max(abs(beta))
    SV =  [i  for i in range(n) if scaled_beta[i] > sv_threshold ]   #Indices of support vectors 
    assert len(SV) >0, "Support vectors empty. Tune the threshold"
    beta2 = np.zeros(n);beta2[SV]=beta[SV]
    return beta2, b, SV, primal_optimal_value

def primal_robust_l2_depol(K_star, y_train, delta_1 = 1e-2, delta_2 = 1e-2, C = 1, ci=0, shots = 20, circuit_type = 'gates',sv_threshold=5e-2,numerical_shift=1e-7):
    n = K_star.shape[1]
    N = 2*n + 2
    g = 0.5
#    if abs(eigmin(K_star))<numerical_shift:
#        K_star = K_star + numerical_shift*np.eye(n)
    if circuit_type == 'swap':
        g = 1
    if ci!=0:
        sqTr = g*kappa(delta_1)/ci
        Tr = int(sqTr**2)
        print("T=",Tr)
    else:
        Tr=0
    K = stochastic_kernel_matrix(K_star,shots=Tr,circuit_type =circuit_type)
    x = cp.Variable(N)
    #The SOCP Constraint:
    A1 = np.zeros((N,N))
    b1 = np.zeros(N)
    ...
    Ai = np.zeros((n,N))
    Ai[:n,:n] = np.eye(n)
    Ai[:n,n:2*n]=np.multiply(y_train,K).T#np.diag(y_train)@K_star#
    Ai[:n,N-2] = y_train
    Ai[:n,N-1] = -(kappa(delta_1)*(g/np.sqrt(shots))+ci)*np.ones(n)
        #linear_constraints += x[i]>= 1 - y_train[i]*(np.dot(x[n:2*n],K_star[:,i]) + x[2*n]) + kappa(delta_1)*(g/np.sqrt(shots))*x[N-1]#cannot be passed this way. See below.
    linear_constraint_2= [Ai@x>=np.ones(n)]
    e = np.zeros(N)
    e[:n]= np.ones(n)
    K_shifted = K + kappa(delta_2)*(g/np.sqrt(shots))* np.eye(n) + ci*np.eye(n)
    print("Minimum eigen value of the estimate:", eigmin(K))
    #K_shifted = 0.5*(K_shifted+K_shifted.T)
    assert isPSD(tik_reg(K_shifted)), "Kernel estimate + ci, is not PSD"
    Q= np.zeros((N,N))
    Q[n:2*n,n:2*n] = tik_reg(K_shifted)
    Q = psd_wrap(Q)
    h_rob = cp.Problem(cp.Minimize(C*e.T@x + 0.5* cp.quad_form(x,Q)) ,socp_constraint + linear_constraint_1 + linear_constraint_2)
    h_rob.solve(solver='MOSEK')
    primal_optimal_value = h_rob.value
...
def primal_robust_l2(K_star, y_train, delta_1 = 1e-2, delta_2 = 1e-2, C = 1, ci=0, shots = 20, circuit_type = 'gates',sv_threshold=5e-2,numerical_shift=1e-7):
    n = K_star.shape[1]
    N = 2*n + 2
    g = 0.5
#    if abs(eigmin(K_star))<numerical_shift:
...
def primal_robust_l1(K_star, y_train, delta_1 = 1e-2, delta_2 = 1e-2, C = 1, shots = 20,ci=0, circuit_type = 'gates',sv_threshold=5e-2,numerical_shift=1e-7):
    n = K_star.shape[1]
    N = 2*n + 2
    g = 0.5
#    if abs(eigmin(K_star))<numerical_shift:
#        K_star = K_star + numerical_shift*np.eye(n)
    ...
    A1[n:2*n,n:2*n] = np.eye(n)
    c1[N-1] = 1
    l1_constraint = [c1.T@x>=cp.norm(A1@x,1)]
    #Linear Constraints
    linear_constraint_1 = [x[i]>=0 for i in range(n)]
    Ai = np.zeros((n,N))
    Ai[:n,:n] = np.eye(n)
    Ai[:n,n:2*n]=np.multiply(y_train,K).T#np.diag(y_train)@K_star#
    Ai[:n,N-2] = y_train
    Ai[:n,N-1] = -ci*np.ones(n) -kappa(delta_1)*(g/np.sqrt(shots))*np.ones(n)
        #linear_constraints += x[i]>= 1 - y_train[i]*(np.dot(x[n:2*n],K_star[:,i]) + x[2*n]) + kappa(delta_1)*(g/np.sqrt(shots))*x[N-1]#cannot be passed this way. See below.
    linear_constraint_2= [Ai@x>=np.ones(n)]
    e = np.zeros(N)
    e[:n]= np.ones(n)
    K_shifted = K + kappa(delta_2)*(g/np.sqrt(shots))* np.eye(n) +ci*np.eye(n)
    print("Minimum eigen value of the estimate:", eigmin(K))
    assert isPSD(tik_reg(K_shifted)), "Not psd after tik-reg"
    Q= np.zeros((N,N))
    Q[n:2*n,n:2*n] = tik_reg(K_shifted)
    Q = psd_wrap(Q)
    h_rob = cp.Problem(cp.Minimize(C*e.T@x + 0.5* cp.quad_form(x,Q)) ,l1_constraint + linear_constraint_1 + linear_constraint_2)
    h_rob.solve(solver='MOSEK')
    primal_optimal_value = h_rob.value
    b = x[2*n].value
    beta = x[n:2*n].value #VECTOR. In previous formalism, symbol beta was used for nXn matrix
...
def primal_robust_l1l2(K_star, y_train, delta_1 = 1e-2, delta_2 = 1e-2, ci = 0.2, T=20,  C = 1, shots = 20, circuit_type = 'gates',sv_threshold=1,numerical_shift=1e-7):
    #T is number of shots for estimate
...
#        K_star = K_star + numerical_shift*np.eye(n)
    ...
    #Replace K_star with its estimate
    K_star = stochastic_kernel_matrix(K_star, shots = T, circuit_type = circuit_type)
    x = cp.Variable(N)
    #The SOCP Constraint:
    A1 = np.zeros((N,N))
    b1 = np.zeros(N)
...
#    l1_constraint = [cp.norm()]
    ...
    Ai[:n,n:2*n]=np.multiply(y_train,K_star).T#np.diag(y_train)@K_star#
    Ai[:n,N-3] = y_train
    Ai[:n,N-2] = -kappa(delta_1)*(g/np.sqrt(shots))*np.ones(n)
    Ai[:n,N-1] = -ci
        #linear_constraints += x[i]>= 1 - y_train[i]*(np.dot(x[n:2*n],K_star[:,i]) + x[2*n]) + kappa(delta_1)*(g/np.sqrt(shots))*x[N-1]#cannot be passed this way. See below.
    linear_constraint_2= [Ai@x>=np.ones(n)]
    e = np.zeros(N)
    e[:n]= np.ones(n)
    K_shifted = K_star + (kappa(delta_2)*(g/np.sqrt(shots))+ ci)* np.eye(n)
    assert isPSD(K_shifted), " The estimate matrix + shift, is not psd"
    Q= np.zeros((N,N))
    Q[n:2*n,n:2*n] = K_shifted
    Q = psd_wrap(Q)
    h_rob = cp.Problem(cp.Minimize(C*e.T@x + 0.5* cp.quad_form(x,Q)) ,socp_constraint +l1_constraint + linear_constraint_1 + linear_constraint_2)
    h_rob.solve(solver='MOSEK')
    primal_optimal_value = h_rob.value
    b = x[2*n].value
    beta = x[n:2*n].value #VECTOR. In previous formalism, symbol beta was used for nXn matrix
...
def dual_robust_quadratic(K_star, y_train, delta=0.1, C = np.inf, shots = 20, circuit_type = 'gates',sv_threshold = 1e-2,numerical_shift = 1e-7):
    g = 1
    n= K_star.shape[1]
    if abs(eigmin(K_star))<numerical_shift:#Ensure the nominal program behaves properly
        K_star = K_star + numerical_shift*np.eye(n)
    if circuit_type == 'swap':
        g = 2
    x = cp.Variable(n)
    ...
    Y = np.diag(y_train)
    Q = Y@K_rob_reg@Y
    Q = psd_wrap(Q) #skip PSD CHECKS of cvxpy.
    ...
    for i in SV:
        ...
        b += s
    alpha2 = np.zeros(n);alpha2[SV] = alpha[SV]
    return alpha2, b/len(SV), SV#, dual_optimal_value

def primal_robust_socp_no_dummy(K_star, y_train, delta_1 = 1e-2, delta_2 = 1e-2, 
                        C = 1, shots = 20, circuit_type = 'gates', 
                        sv_threshold = 5e-2, numerical_shift = 1e-6): 
    g = 1
    if circuit_type == 'swap':
        g = 2
...
def margin_error(test_train_kernel,y_test, beta, b, margin=1):
    N_test = test_train_kernel.shape[0]
    N_train = test_train_kernel.shape[1]
    assert N_train == len(beta), "N_train and beta are mismatched"
    F = np.zeros(N_test)
    for l_test in range(N_test):
        K = test_train_kernel[l_test,:]
        F[l_test] = np.dot(beta,K) + b
    errors = y_test*F - margin*np.ones(N_test)
    margin_violations =np.array(errors<=0)
    #print("Number of Margin Violations:",np.count_nonzero(margin_violations))
    return np.count_nonzero(margin_violations)/N_test


def classifier_dual(test_train_kernel, alpha, y, b,SV):
    return kernel_matrix_classifier(test_train_kernel[:,SV],alpha[SV],y[SV],b)

def classifier_alt_dual(test_train_kernel,alpha,y,b):
    return kernel_matrix_classifier(test_train_kernel,alpha,y,b)

def primal_value(y,beta,K,b,C=1):
    L_array = 1 - np.multiply(y,beta@K +b)
    L = 0
    for i in L_array:
        L = L + max(i,0)
    return C*L + 0.5*beta.T@K@beta
